...
async def process_tender(
    ...
    try:
        logger.info(
            "Processing tender: id=%s user=%s file=%s size=%d",
            tender_id,
        ...
        # Upload original file to R2 (best-effort; failures don't block the response)
        r2_key = r2_storage.make_key(tender_id, tender_file.filename)
        uploaded = await asyncio.to_thread(r2_storage.upload_file, tmp_path, r2_key)
        if not uploaded:
            r2_key = ""
            logger.debug("R2 upload skipped (not configured or failed): tender_id=%s", tender_id)
    ...
    logger.info("Tender processed successfully: id=%s ocr_used=%s", tender_id, ocr_used)
